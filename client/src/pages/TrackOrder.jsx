...
import { Search, CheckCircle2, Clock, Truck, Package, PackageCheck, XCircle } from "lucide-react";
...
function TrackOrder() {
  ...
  useEffect(() => {
    ...
    orderService
      ...
      .catch((err) => {
        if (live) {
          setOrder(null);
          setError(getErrorMessage(err) || "Order not found. Please check your order reference number.");
        }
      })
      .finally(() => {
  ...
  const isTerminalNegative = order && ["CANCELLED", "RETURNED", "REFUNDED"].includes(order.orderStatus);
  const currentStepIndex = order && ORDER_FLOW_INDEX[order.orderStatus] !== undefined ? ORDER_FLOW_INDEX[order.orderStatus] : 0;
    ...
    <Container className="py-14 max-w-3xl">
      ...
      <div className="text-center">
        ...
        <p className="mt-3 text-sm text-stone-600">
          Enter your order reference number (e.g. RB-2026-00001) to view status and timeline.
        </p>
      </div>

      <form onSubmit={handleSubmit} className="mt-8 flex gap-3 max-w-lg mx-auto">
          ...
          <div className="rounded-lg border border-stone-200 bg-white p-6 sm:p-8 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-4 border-b border-stone-200 pb-5">
              <div>
                ...
                <p className="text-lg font-bold font-serif text-stone-900">{order.orderNumber}</p>
              </div>
              <div>
            ...
            {isTerminalNegative ? (
              <div className="mt-8 rounded-lg bg-red-50 p-5 flex items-center gap-4 text-red-800">
                ...
                <div>
                  <p className="font-semibold">This order has been {order.orderStatus.toLowerCase()}</p>
                  <p className="text-xs text-red-600 mt-0.5">
                    If you have questions, please reach out to our bridal concierge team.
                  </p>
                </div>
              </div>
            ) : (
                      ...
                      <div key={step.key} className="flex flex-col items-center text-center z-10">
                        ...
                        <span
                          className={`mt-2 text-[11px] sm:text-xs font-medium max-w-18 ${
                            isCurrent ? "font-bold text-[#7d2034]" : isDone ? "text-stone-900" : "text-stone-400"
                          }`}
                        >
                          {step.label}
                        </span>
            ...
            {Array.isArray(order.timeline) && order.timeline.length > 0 && (
              <div className="mt-10 border-t border-stone-200 pt-6">
                <h3 className="text-sm font-semibold text-stone-900 mb-4">Activity Timeline</h3>
            ...
            <div className="mt-8 border-t border-stone-200 pt-6">
              ...
              <div className="mt-4 flex justify-between border-t border-stone-100 pt-3 text-sm font-bold text-stone-900">
                ...
                <span className="font-serif text-lg text-[#7d2034]">₹{order.totalAmount}</span>
              </div>
            </div>
          </div>
        </div>
