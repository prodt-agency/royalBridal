...
function Footer() {
  return (
    <footer className="bg-[#24181a] pb-7 pt-14 text-stone-300">
      <Container>
        <div className="grid gap-10 border-b border-white/15 pb-12 sm:grid-cols-2 md:grid-cols-4">
          <div className="md:col-span-1">
            <p className="font-serif text-3xl text-white">Royal Bridal</p>
            <p className="mt-4 text-xs sm:text-sm leading-6 text-stone-400">
          ...
          <div>
            ...
            <a className="mt-4 flex items-center gap-2 text-xs text-stone-400 hover:text-white" href={`tel:${SITE.PHONE.replaceAll(" ", "")}`}>
              <Phone size={14} /> {SITE.PHONE}
            </a>
            <a className="mt-3 flex items-center gap-2 text-xs text-stone-400 hover:text-white" href={`mailto:${SITE.EMAIL}`}>
              <Mail size={14} /> {SITE.EMAIL}
            </a>
          </div>
          <div>
            <h2 className="text-xs font-bold uppercase tracking-[.2em] text-white">Administration</h2>
            <Link to="/admin/login" className="mt-4 inline-flex items-center gap-1.5 text-xs text-[#e6c98c] hover:underline">
